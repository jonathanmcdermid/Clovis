...
constexpr auto iid_table = [] {
  ...
  for (int depth = 1; depth <= MAX_PLY; ++depth)
    for (int i = 0; i < 2; ++i)
      arr[i][depth] =
          (iid_factor[i] * depth - iid_reduction[i]) / (iid_divisor[i] + 1);
...
int quiescence(Position &pos, int alpha, int beta, uint64_t &nodes,
  ...
  move_pick::MovePicker mp(pos, 0, MOVE_NONE,
...
int negamax(Position &pos, int alpha, int beta, int depth, const int ply,
  ...
  if (!in_check) {
    ...
    // if evaluation is above a certain threshold, we can trust that it will
    ...
    if (!PV_NODE && !NULL_NODE && depth >= null_depth &&
      ...
      pos.do_null_move();
      Line line;
      score = -negamax<NODE_NULL>(pos, -beta, -beta + 1, depth - null_reduction,
                                  ply + 1, MOVE_NULL, nodes, line);
      pos.undo_null_move();
    ...
    if (tte.key != pos.bs->key && depth >= iid_depth[PV_NODE]) {
      ...
      Line line;
      negamax<N == NODE_NULL ? NODE_NON_PV : N>(pos, alpha, beta,
                                                iid_table[PV_NODE][depth], ply,
                                                prev_move, nodes, line);
  ...
  while (const Move curr_move = mp.get_next(play_quiets)) {
    ...
    if (pos.is_draw())
      ...
      if (moves_searched > 1 && depth > lmr_depth &&
        ...
        R = std::clamp(R - std::clamp(history_entry / lmr_history_divisor,
                                      -lmr_history_min, lmr_history_max),
                       0, depth - lmr_reduction);
        // search current move with reduced depth:
        score = -negamax<NODE_NON_PV>(pos, -alpha - 1, -alpha, depth - R - 1,
                                      ply + 1, curr_move, nodes, line);
        // if search does not fail low, we search again without reduction
        if (R && score > alpha)
          score = -negamax<NODE_NON_PV>(pos, -alpha - 1, -alpha, depth - 1,
                                        ply + 1, curr_move, nodes, line);
      ...
      if (PV_NODE && (moves_searched == 1 ||
        ...
        score = -negamax<NODE_PV>(pos, -beta, -alpha, depth - 1, ply + 1,
...
void start_search(Position &pos, const SearchLimits &limits, SearchInfo &info) {
  ...
  if (ml.size() > 1) {
    ...
    allocated_time = limits.depth
                         ? LLONG_MAX
                         : 5 * limits.time[pos.side] / (limits.moves_left + 5);
    ...
    for (int depth = 1;
         depth <= MAX_PLY && (limits.depth == 0 || depth <= limits.depth);
         ++depth) {

      info.score = negamax<NODE_ROOT>(pos, alpha, beta, depth, 0, MOVE_NONE,
      ...
      std::cout << "info depth " << std::setw(2) << depth << " score cp "
                << std::setw(4) << info.score << " nodes " << std::setw(8)
                << info.nodes << " time " << std::setw(6) << elapsed_time
                << " nps " << std::setw(8)
      ...
      if (elapsed_time > allocated_time / 3)
        break;
