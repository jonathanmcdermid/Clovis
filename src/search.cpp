...
namespace clovis::search {
    ...
    constexpr double log_table[] = {
        0.000000, 0.000000, 0.693147, 1.098612, 1.386294, 1.609438, 1.791759,
        1.945910, 2.079442, 2.197225, 2.302585, 2.397895, 2.484907, 2.564949,
        2.639057, 2.708050, 2.772589, 2.833213, 2.890372, 2.944439, 2.995732,
        3.044522, 3.091042, 3.135494, 3.178054, 3.218876, 3.258097, 3.295837,
        3.332205, 3.367296, 3.401197, 3.433987, 3.465736, 3.496508, 3.526361,
        3.555348, 3.583519, 3.610918, 3.637586, 3.663562, 3.688879, 3.713572,
        3.737670, 3.761200, 3.784190, 3.806662, 3.828641, 3.850148, 3.871201,
        3.891820, 3.912023, 3.931826, 3.951244, 3.970292, 3.988984, 4.007333,
        4.025352, 4.043051, 4.060443, 4.077537, 4.094345, 4.110874, 4.127134,
        4.143135, 4.158883};
    ...
    constexpr auto lmr_table = [] {
        ...
        for (int depth = 1; depth <= MAX_PLY; ++depth)
            for (int ordered = 1; ordered < 64; ++ordered)
                arr[depth][ordered] = static_cast<int>(
                    0.75 + log_table[depth] * log_table[ordered] / 2.25);
    ...
    constexpr auto iid_table = [] {
        ...
        for (int depth = 1; depth <= MAX_PLY; ++depth)
            for (int i = 0; i < 2; ++i)
                arr[i][depth] = (iid_factor[i] * depth - iid_reduction[i]) /
                                (iid_divisor[i] + 1);
    ...
    int quiescence(Position &pos, int alpha, int beta, uint64_t &nodes,
        ...
        if (!in_check) {
            ...
            if (tte.key == pos.bs->key &&
                ((tte.flags == HASH_ALPHA) == (tte.eval < eval)))
                eval = tte.eval;
            if (eval >= beta) return beta;
            if (eval > alpha) alpha = eval;
        }

        move_pick::MovePicker mp(
            pos, 0, MOVE_NONE, (tte.key == pos.bs->key) ? tte.move : MOVE_NONE);
        ...
        while (const Move curr_move = mp.get_next(in_check)) {
            ...
            // fail high
            if (eval >= beta) {
                transposition::new_entry(pos.bs->key, 0, beta, HASH_BETA,
                                         curr_move);
                return beta;
            }
            if (eval > best_eval) {
        ...
        transposition::new_entry(pos.bs->key, 0, alpha,
                                 alpha > old_alpha ? HASH_EXACT : HASH_ALPHA,
                                 best_move);
    ...
    template <NodeType N>
    int negamax(Position &pos, int alpha, int beta, int depth, const int ply,
                const Move prev_move, uint64_t &nodes, Line &pv_line) {
        ...
        if (!in_check) {
            int score =
                tte.key == pos.bs->key ? tte.eval : eval::evaluate<false>(pos);
            ...
            // null move pruning
            if (!PV_NODE && !NULL_NODE && depth >= null_depth &&
                pos.stm_has_promoted()) {
                pos.do_null_move();
                Line line;
                score = -negamax<NODE_NULL>(pos, -beta, -beta + 1,
                                            depth - null_reduction, ply + 1,
                                            MOVE_NULL, nodes, line);
                pos.undo_null_move();
            ...
            if (tte.key != pos.bs->key && depth >= iid_depth[PV_NODE]) {
                ...
                negamax<N == NODE_NULL ? NODE_NON_PV : N>(
                    pos, alpha, beta, iid_table[PV_NODE][depth], ply, prev_move,
                    nodes, line);
        ...
        move_pick::MovePicker mp(pos, ply, prev_move,
                                 tte.key == pos.bs->key ? tte.move : MOVE_NONE);
        Move                  best_move  = MOVE_NONE;
        ...
        while (const Move curr_move = mp.get_next(play_quiets)) {
            ...
            if (pos.is_draw())
                ...
                if (moves_searched > 1 && depth > lmr_depth &&
                    move_capture(curr_move) == NO_PIECE &&
                    move_promotion_type(curr_move) == NO_PIECE) {
                    const int history_entry =
                        move_pick::get_history_entry(~pos.side, curr_move);
                    // reduction factor
                    ...
                    R = std::clamp(
                        R - std::clamp(history_entry / lmr_history_divisor,
                                       -lmr_history_min, lmr_history_max),
                        0, depth - lmr_reduction);
                    // search current move with reduced depth:
                    score = -negamax<NODE_NON_PV>(pos, -alpha - 1, -alpha,
                                                  depth - R - 1, ply + 1,
                                                  curr_move, nodes, line);
                    ...
                    score = -negamax<NODE_NON_PV>(pos, -alpha - 1, -alpha,
                                                  depth - 1, ply + 1, curr_move,
                                                  nodes, line);
                // full PV search if all options are exhausted
                if (PV_NODE && (moves_searched == 1 ||
                                ((ROOT_NODE || score < beta) && score > alpha)))
                    score = -negamax<NODE_PV>(pos, -beta, -alpha, depth - 1,
                                              ply + 1, curr_move, nodes, line);
            }
            ...
            if (score >= beta) {
                if (move_capture(curr_move) == NO_PIECE) {
                    ...
                    move_pick::update_counter_entry(pos.side, prev_move,
                                                    curr_move);
                }
                transposition::new_entry(pos.bs->key, depth, beta, HASH_BETA,
                                         curr_move);
                return beta;
            }
            ...
            if (!ROOT_NODE && score > -MIN_CHECKMATE_SCORE &&
        ...
        transposition::new_entry(pos.bs->key, depth, best_score, hash_flag,
                                 best_move);
    ...
    void start_search(Position &pos, const SearchLimits &limits,
        ...
        if (ml.size() > 1) {
            ...
            allocated_time = limits.depth ? LLONG_MAX
                                          : 5 * limits.time[pos.side] /
                                                (limits.moves_left + 5);
            ...
            for (int depth = 1; depth <= MAX_PLY &&
                                (limits.depth == 0 || depth <= limits.depth);
                 ++depth) {
                info.score =
                    negamax<NODE_ROOT>(pos, alpha, beta, depth, 0, MOVE_NONE,
                                       info.nodes, info.pv_line);
                ...
                std::cout << "info depth " << std::setw(2) << depth
                          ...
                          << " nodes " << std::setw(8) << info.nodes << " time "
                          << std::setw(6) << elapsed_time << " nps "
                          << std::setw(8)
                          << 1000ULL * info.nodes / (elapsed_time + 1)
                          << " pv ";
