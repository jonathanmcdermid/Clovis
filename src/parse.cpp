...
Move parse(const Position &pos, std::string move) {
    ...
    if (move.find("O-O") != std::string::npos)
        return encode_move(
            relative_square(pos.side, E1), relative_square(pos.side, move == "O-O" ? G1 : C1),
            make_piece(KING, pos.side), NO_PIECE, false, false, false, true);
    if (islower(move[0])) { // pawn moves
        const Piece promo =
            move[move.length() - 2] == '='
                ? make_piece(
                      static_cast<PieceType>(piece_str.find(move[move.length() - 1])), pos.side)
                : NO_PIECE;
        const Square to = (promo == NO_PIECE) ? str2sq(move.substr(move.length() - 2))
        ...
        return encode_move(
            from, to, make_piece(PAWN, pos.side), promo, move.find('x') != std::string::npos,
    ...
    Bitboard bb =
        bitboards::get_attacks(piece_type(piece), pos.occ_bb[BOTH], to) & pos.pc_bb[piece];
    ...
    return encode_move(
        from, to, piece, NO_PIECE, move.find('x') != std::string::npos, false, false, false);
}
