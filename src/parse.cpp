...
Move parse(const Position &pos, std::string move) {
  ...
  if (move.find("O-O") != std::string::npos)
    return encode_move(relative_square(pos.side, E1),
                       ...
                       make_piece(KING, pos.side), NO_PIECE, false, false,
                       false, true);
  if (islower(move[0])) { // pawn moves
    const Piece promo = move[move.length() - 2] == '='
                            ? make_piece(static_cast<PieceType>(piece_str.find(
                                             move[move.length() - 1])),
                                         pos.side)
                            : NO_PIECE;
    ...
    const Square from = (move[1] == 'x')
                            ? make_square(static_cast<File>(move[0] - 'a'),
                                          rank_of(to - pawn_push(pos.side)))
                        : pos.pc_table[to - pawn_push(pos.side)] == NO_PIECE
                            ? to - 2 * pawn_push(pos.side)
                            : to - pawn_push(pos.side);

    return encode_move(from, to, make_piece(PAWN, pos.side), promo,
                       move.find('x') != std::string::npos,
                       abs(rank_of(to) - rank_of(from)) == 2,
                       pos.bs->en_passant == to, false);
  }
  // major moves
  const Piece piece =
      make_piece(static_cast<PieceType>(piece_str.find(move[0])), pos.side);
  const Square to = str2sq(move.substr(move.length() - 2));
  Bitboard bb =
      bitboards::get_attacks(piece_type(piece), pos.occ_bb[BOTH], to) &
      pos.pc_bb[piece];
  ...
  return encode_move(from, to, piece, NO_PIECE,
                     move.find('x') != std::string::npos, false, false, false);
}
...
void generate_data() {
  ...
  while (!ifs.eof()) {
    ...
    while (getline(ifs, line))
      if (line.find(std::to_string(pos.bs->fmc) + "... ") !=
              std::string::npos ||
    ...
    do {
      ...
      while (true) {
        ...
        if (token.find('.') == std::string::npos) {
          ...
          if (pos.bs->fmc > 8 && token[token.length() - 1] != '#' &&
            ...
            if (info.score < MIN_CHECKMATE_SCORE &&
                info.score > -MIN_CHECKMATE_SCORE) {
              for (const auto &it : info.pv_line)
                if (!pos.do_move(it))
                  exit(EXIT_FAILURE);

              if (std::ranges::find(keys.begin(), keys.end(), pos.bs->key) ==
                  keys.end()) {
                if (const int eval = pos.side == WHITE
              ...
              std::for_each(
                  std::make_reverse_iterator(info.pv_line.last),
