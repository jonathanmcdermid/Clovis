...
namespace clovis::parse {
    ...
    Move parse(const Position &pos, std::string move) {
        ...
        if (move.find("O-O") != std::string::npos)
            return encode_move(
                relative_square(pos.side, E1),
                relative_square(pos.side, move == "O-O" ? G1 : C1),
                make_piece(KING, pos.side), NO_PIECE, false, false, false,
                true);
        if (islower(move[0])) { // pawn moves
            const Piece promo =
                move[move.length() - 2] == '='
                    ? make_piece(static_cast<PieceType>(
                                     piece_str.find(move[move.length() - 1])),
                                 pos.side)
                    : NO_PIECE;
        ...
        // major moves
        const Piece piece = make_piece(
            static_cast<PieceType>(piece_str.find(move[0])), pos.side);
        ...
        return encode_move(from, to, piece, NO_PIECE,
                           move.find('x') != std::string::npos, false, false,
                           false);
    }
        ...
        while (!ifs.eof()) {
            ...
            while (getline(ifs, line))
                if (line.find(std::to_string(pos.bs->fmc) + "... ") !=
                        std::string::npos ||
                    line.find(std::to_string(pos.bs->fmc) + ". ") !=
                        std::string::npos)
                    break;
                    ...
                    if (token.find('.') == std::string::npos) {
                        ...
                        if (pos.bs->fmc > 8 &&
                            ...
                            if (info.score < MIN_CHECKMATE_SCORE &&
                                ...
                                if (std::ranges::find(keys.begin(), keys.end(),
                                                      pos.bs->key) ==
                                    keys.end()) {
                                    if (const int eval =
                                            pos.side == WHITE
