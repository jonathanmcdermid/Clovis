...
namespace clovis {
    ...
    struct Position {
        explicit Position(const char *fen)
            : side{WHITE}, bs{nullptr}, pc_table{}, pc_bb{}, occ_bb{} {
            set(fen);
        }
    ...
    bool Position::is_attacked(const Square sq) const {
        return (
            ...
            (pc_bb[make_piece(QUEEN, ~US)] &
             bitboards::get_attacks<QUEEN>(occ_bb[BOTH], sq)) ||
    ...
    inline Bitboard Position::consider_xray(const Bitboard occ, const Square to,
                                            const PieceType pt) const {
        return (pt == PAWN || pt == BISHOP)
                   ? occ & (bitboards::get_attacks<BISHOP>(occ, to) &
                            (pc_bb[W_QUEEN] | pc_bb[B_QUEEN] | pc_bb[W_BISHOP] |
                             pc_bb[B_BISHOP]))
               : pt == ROOK  ? occ & (bitboards::get_attacks<ROOK>(occ, to) &
                                     (pc_bb[W_QUEEN] | pc_bb[B_QUEEN] |
                                      pc_bb[W_ROOK] | pc_bb[B_ROOK]))
               : pt == QUEEN ? consider_xray(occ, to, BISHOP) |
                                   consider_xray(occ, to, ROOK)
                             : 0ULL;
    }
    ...
    inline Bitboard Position::attackers_to(const Square sq) const {
        return (bitboards::pawn_attacks[BLACK][sq] & pc_bb[W_PAWN]) |
               ...
               (bitboards::knight_attacks[sq] &
                (pc_bb[W_KNIGHT] | pc_bb[B_KNIGHT])) |
               (bitboards::king_attacks[sq] & (pc_bb[W_KING] | pc_bb[B_KING])) |
               (bitboards::get_attacks<ROOK>(occ_bb[BOTH], sq) &
                (pc_bb[W_QUEEN] | pc_bb[B_QUEEN] | pc_bb[W_ROOK] |
                 pc_bb[B_ROOK])) |
               (bitboards::get_attacks<BISHOP>(occ_bb[BOTH], sq) &
                (pc_bb[W_QUEEN] | pc_bb[B_QUEEN] | pc_bb[W_BISHOP] |
                 pc_bb[B_BISHOP]));
    ...
    inline bool Position::stm_has_promoted() const {
        return pc_bb[make_piece(KNIGHT, side)] |
               pc_bb[make_piece(BISHOP, side)] | pc_bb[make_piece(ROOK, side)] |
