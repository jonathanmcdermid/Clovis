...
T *generate_majors(const Position &pos, T *moves) {
  ...
  while (bb) {
    ...
    while (att) {
      ...
      *moves++ = encode_move(src, tar, make_piece(PT, US), NO_PIECE,
                             M == QUIET_MOVES ? false : pos.occ_bb[BOTH] & tar,
                             false, false, false);
    }
  }
...
T *generate_promotions(T *moves, const Square src, const Square tar) {
  ...
  if constexpr (M != QUIET_MOVES)
    *moves++ = encode_move(src, tar, make_piece(PAWN, US),
  ...
  if constexpr (M != CAPTURE_MOVES) {
    ...
    *moves++ = encode_move(src, tar, make_piece(PAWN, US), make_piece(ROOK, US),
...
T *generate_all(const Position &pos, T *moves) {
  ...
  while (bb) {
    ...
    if (rank_of(src) == relative_rank(US, RANK_7)) {
      ...
      if (QUIETS && !(pos.occ_bb[BOTH] & tar)) {
        // single push
        *moves++ = encode_move(src, tar, make_piece(PAWN, US), NO_PIECE, false,
                               false, false, false);
        // double push
        if (rank_of(src) == relative_rank(US, RANK_2) &&
            !(pos.occ_bb[BOTH] & (tar + pawn_push(US))))
          *moves++ = encode_move(src, tar + pawn_push(US), make_piece(PAWN, US),
                                 NO_PIECE, false, true, false, false);
      }
      if constexpr (CAPTURES) {
        ...
        while (att) {
          tar = bitboards::pop_lsb(att);
          *moves++ = encode_move(src, tar, make_piece(PAWN, US), NO_PIECE, true,
                                 false, false, false);
        }

        if (bitboards::pawn_attacks[US][src] & pos.bs->en_passant)
          *moves++ = encode_move(src, pos.bs->en_passant, make_piece(PAWN, US),
                                 NO_PIECE, true, false, true, false);
      }
    }
  }
  ...
  if (QUIETS && !pos.is_attacked<US>(relative_square(US, E1))) {
    ...
    if (pos.bs->castle & ks_castle_rights(US) &&
      ...
      *moves++ = encode_move(relative_square(US, E1), relative_square(US, G1),
                             make_piece(KING, US), NO_PIECE, false, false,
                             false, true);
    ...
    if (pos.bs->castle & qs_castle_rights(US) &&
      ...
      *moves++ = encode_move(relative_square(US, E1), relative_square(US, C1),
                             make_piece(KING, US), NO_PIECE, false, false,
                             false, true);
  }
...
template ScoredMove *generate<ScoredMove, QUIET_MOVES>(const Position &pos,
                                                       ScoredMove *moves);
template ScoredMove *generate<ScoredMove, CAPTURE_MOVES>(const Position &pos,
                                                         ScoredMove *moves);
