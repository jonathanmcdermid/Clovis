...
namespace clovis {
    ...
    constexpr std::string_view symbols[] = {"·", "♙", "♘", "♗", "♖",
                                            ...
                                            "♞", "♝", "♜", "♛", "♚"};
#else
    constexpr std::string_view symbols[] = {".", "P", "N", "B", "R",
                                            "Q", "K", "",  "",  "p",
                                            "k", "b", "r", "q", "k"};
#endif
    ...
    constexpr auto castling_rights = [] {
        ...
        for (const Colour c : {WHITE, BLACK}) {
            arr[relative_square(c, E1)] &=
                ~(ks_castle_rights(c) | qs_castle_rights(c));
    ...
    template <Colour US>
    Square Position::get_pinner(const Square sq) const {
        if (const Square ksq = bitboards::lsb(pc_bb[make_piece(KING, US)]);
            bitboards::get_attacks<QUEEN>(ksq) & sq) {
            Bitboard candidates =
                ((bitboards::get_attacks<ROOK>(occ_bb[BOTH] ^ sq, ksq) &
                  (pc_bb[make_piece(QUEEN, ~US)] |
                   pc_bb[make_piece(ROOK, ~US)])) |
                 (bitboards::get_attacks<BISHOP>(occ_bb[BOTH] ^ sq, ksq) &
                  (pc_bb[make_piece(QUEEN, ~US)] |
                   pc_bb[make_piece(BISHOP, ~US)])));

            while (candidates)
                if (const Square candidate = bitboards::pop_lsb(candidates);
                    bitboards::between_squares(ksq, candidate) & sq)
                    return candidate;
        }
    ...
    bool Position::discovery_threat(const Square sq) const {
        ...
        Bitboard their_immobile_pawns =
            (bitboards::shift<pawn_push(US)>(occ_bb[BOTH]) &
             pc_bb[make_piece(PAWN, ~US)]) &
            ~(bitboards::shift<pawn_push(US) + EAST>(occ_bb[US]) |
              bitboards::shift<pawn_push(US) + WEST>(occ_bb[US]));

        if (side == ~US && bs->en_passant != SQ_NONE)
            their_immobile_pawns &=
                ~bitboards::pawn_attacks[US][bs->en_passant];

        Bitboard candidates =
            ((bitboards::get_attacks<ROOK>(pc_bb[W_PAWN] | pc_bb[B_PAWN], sq) &
              (pc_bb[make_piece(ROOK, ~US)])) |
             (bitboards::get_attacks<BISHOP>(
                  pc_bb[make_piece(PAWN, US)] | their_immobile_pawns, sq) &
              (pc_bb[make_piece(BISHOP, ~US)])));

        const Bitboard occupancy = occ_bb[BOTH] ^ candidates;

        while (candidates)
            if (std::popcount(bitboards::between_squares(
                                  sq, bitboards::pop_lsb(candidates)) &
                              occupancy) == 1)
                return true;
    ...
    std::string Position::get_fen() const {
        ...
        for (Rank r = RANK_8; r >= RANK_1; --r) {
            ...
            for (File f = FILE_A; f <= FILE_H; ++f) {
                if (const Square sq = make_square(f, r);
        ...
        fen << " " + (bs->en_passant == SQ_NONE ? "-" : sq2str(bs->en_passant))
            << " " << std::to_string(bs->hmc) << " " << std::to_string(bs->fmc);
    ...
    void Position::set(const char *fen) {
        ...
        while ((ss >> token) && !isspace(token)) {
            if (isdigit(token))
                ...
                sq = sq + 2 * SOUTH;
            else if (size_t index = piece_str.find(token);
    ...
    Key Position::make_pawn_key() const {
        ...
        for (Square sq = SQ_ZERO; sq < SQ_N; ++sq)
            if (piece_type(pc_table[sq]) == PAWN ||
                piece_type(pc_table[sq]) == KING)
                k ^= zobrist::piece_square[pc_table[sq]][sq];
    ...
    bool Position::do_move(const Move move) {
        ...
        if (move_capture(move)) {
            if (move_en_passant(move)) {
                ...
                bs->pawn_key ^=
                    zobrist::piece_square[bs->captured_piece][victim_sq];
                remove_piece(victim_sq);
                put_piece(piece, tar);
            } else {
                if (piece_type(bs->captured_piece) == PAWN)
                    bs->pawn_key ^=
                        zobrist::piece_square[bs->captured_piece][tar];
    ...
    // attackers
    Square Position::get_smallest_attacker(const Bitboard attackers,
                                           const Colour   stm) const {
    ...
    bool Position::is_repeat() const {
        ...
        for (int end = std::min(bs->hmc, bs->ply_null); end >= 4; end -= 4) {
            assert(temp->prev && temp->prev->prev && temp->prev->prev->prev &&
