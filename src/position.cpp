...
constexpr std::string_view symbols[] = {"·", "♙", "♘", "♗", "♖",
                                        ...
                                        "♞", "♝", "♜", "♛", "♚"};
#else
constexpr std::string_view symbols[] = {".", "P", "N", "B", "R", "Q", "K", "",
                                        "",  "p", "k", "b", "r", "q", "k"};
#endif
  ...
  if (const Square ksq = bitboards::lsb(pc_bb[make_piece(KING, US)]);
    ...
    Bitboard candidates =
        ((bitboards::get_attacks<ROOK>(occ_bb[BOTH] ^ sq, ksq) &
          (pc_bb[make_piece(QUEEN, ~US)] | pc_bb[make_piece(ROOK, ~US)])) |
         (bitboards::get_attacks<BISHOP>(occ_bb[BOTH] ^ sq, ksq) &
          (pc_bb[make_piece(QUEEN, ~US)] | pc_bb[make_piece(BISHOP, ~US)])));
...
template <Colour US> bool Position::discovery_threat(const Square sq) const {
  ...
  // we don't have to worry about shift because discovery pawns will never be on
  // outer files
  ...
  while (candidates)
    if (std::popcount(
            bitboards::between_squares(sq, bitboards::pop_lsb(candidates)) &
            occupancy) == 1)
      return true;
