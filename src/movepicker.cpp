...
namespace clovis::move_pick {
        ...
        switch (stage) {
            case TT_MOVE:
                ...
                if (tt_move != MOVE_NONE &&
                    (play_quiets || move_capture(tt_move) ||
                     move_promotion_type(tt_move)))
                    return tt_move;
                [[fallthrough]];
            case INIT_CAPTURES:
                curr = last_bad_cap = moves.data();
                last = move_gen::generate<ScoredMove, CAPTURE_MOVES>(
                    pos, moves.data());
                score_captures();
            ...
            case INIT_QUIETS:
                if (play_quiets) {
                    curr = last_bad_cap;
                    last =
                        move_gen::generate<ScoredMove, QUIET_MOVES>(pos, curr);
                    score_quiets();
    ...
    void MovePicker::score_captures() {
        for (auto &sm : std::ranges::subrange(moves.data(), last))
            ...
            sm.score =
                mvv_lva[move_piece_type(sm)][pos.pc_table[move_to_sq(sm)]] +
