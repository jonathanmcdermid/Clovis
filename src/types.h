...
constexpr auto piece_value =
    std::array{0, 100, 300, 300, 500, 900, 20000, 0, 0, 100, 300, 300, 500, 900, 20000};
...
constexpr Move encode_move(
    const Square from, const Square to, const Piece piece, const Piece promo, const bool cap,
    const bool dub, const bool ep, const bool cast) {
...
constexpr char sq_names[65][3] = {"a1", "b1", "c1", "d1", "e1", "f1", "g1", "h1", "a2", "b2", "c2",
...
inline std::string move2str(const Move m) {
    return (move_promotion_type(m)) ? sq2str(move_from_sq(m)) + sq2str(move_to_sq(m)) +
