...
constexpr auto piece_value = std::array{0, 100, 300, 300, 500, 900, 20000, 0,
...
constexpr Move encode_move(const Square from, const Square to,
                           const Piece piece, const Piece promo, const bool cap,
                           const bool dub, const bool ep, const bool cast) {
  return static_cast<Move>(from | (to << 6) | (piece << 12) | (promo << 16) |
                           (cap << 20) | (dub << 21) | (ep << 22) |
                           (cast << 23));
}
