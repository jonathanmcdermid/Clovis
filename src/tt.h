...
namespace clovis::transposition {
    ...
    struct TTEntry {
        ...
        TTEntry(const Key k, const int d, const HashFlag f, const int e,
                const Move m)
    ...
    static std::unique_ptr<TTBucket[]> ht =
        std::make_unique<TTBucket[]>(tt_size);
    static std::unique_ptr<PTEntry[]> pt = std::make_unique<PTEntry[]>(pt_size);

    static inline void resize(const int mb) {
        tt_size = std::bit_floor(static_cast<size_t>(mb) * 1024 * 1024 /
                                 sizeof(TTBucket));
    ...
    static inline void new_entry(const Key key, const int depth, const int eval,
                                 const HashFlag flags, const Move move) {
        TTBucket &bucket = ht[hash_index(key)];
        bucket[bucket.e1.depth > depth] =
            TTEntry(key, depth, flags, eval, move);
    }
