...
struct TTEntry {
  ...
  TTEntry(const Key k, const int d, const HashFlag f, const int e, const Move m)
      : depth(static_cast<uint8_t>(d)), flags(static_cast<uint8_t>(f)),
        eval(static_cast<int16_t>(e)), move(m), key(k) {}
...
static inline void resize(const int mb) {
  tt_size =
      std::bit_floor(static_cast<size_t>(mb) * 1024 * 1024 / sizeof(TTBucket));
