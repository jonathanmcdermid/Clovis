...
namespace clovis::move_pick {
        ...
        for (const auto c : {WHITE, BLACK})
            for (PieceType p1 = PAWN; p1 <= KING; ++p1)
                for (PieceType p2 = PAWN; p2 <= KING; ++p2)
                    arr[make_piece(p1, c)][make_piece(p2, ~c)] =
                        KING - p1 + KING * p2;
    ...
    inline void update_history_entry(const Move m, const Colour c,
                                     const int bonus) {
        int &history_entry = history_table[cft_index(c, m)];
        history_entry += bonus - (history_entry * abs(bonus) >> 14);
    }

    inline void update_counter_entry(const Colour c, const Move prev,
                                     const Move curr) {
        counter_table[cft_index(c, prev)] = curr;
    }

    inline bool is_killer(const Move m, const int ply) {
        return m == killer_table[ply].primary ||
               m == killer_table[ply].secondary;
    }

    class MovePicker {
       public:
        MovePicker(const Position &p, const int pl, const Move pm,
                   const Move ttm)
            : pos(p),
              ply(pl),
              stage(TT_MOVE),
              curr(moves.data()),
              last(moves.data()),
              last_bad_cap(moves.data()),
              prev_move(pm),
              tt_move(ttm) {}
        Move get_next(bool play_quiets);
        template <HashFlag HF>
        void update_history(Move best_move, int depth) const;
    ...
    template <HashFlag HF>
    void MovePicker::update_history(const Move best_move,
        ...
        for (const auto &sm : std::ranges::subrange(
                 last_bad_cap, HF == HASH_EXACT ? last : curr)) {
