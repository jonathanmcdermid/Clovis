...
inline bool is_doubled_pawn(const Bitboard bb, const Square sq) {
  return bitboards::multiple_bits(bb & bitboards::file_masks[sq]);
}
...
template <Colour US> bool is_candidate_passer(const Position &pos, Square sq) {
  ...
  do {
    if (std::popcount(bitboards::pawn_attacks[US][sq + pawn_push(US)] &
                      pos.pc_bb[make_piece(PAWN, ~US)]) >
        std::popcount(bitboards::pawn_attacks[~US][sq + pawn_push(US)] &
                      pos.pc_bb[make_piece(PAWN, US)]))
      return false;
    sq += pawn_push(US);
...
template <Colour US> bool is_outpost(const Square sq, const EvalInfo &ei) {
  return (outpost_masks[US] & sq & ~ei.potential_pawn_attacks[~US] &
          ei.pawn_attacks[US]);
}

template <Colour US> bool is_fianchetto(const Position &pos, const Square sq) {
  return fianchetto_bishop_mask[US] & sq &&
         center_mask[US] & bitboards::get_attacks<BISHOP>(
                               pos.pc_bb[W_PAWN] | pos.pc_bb[B_PAWN], sq);
}
...
void king_danger(const Bitboard attacks, EvalInfo &ei) {
  ...
  if (or_att_bb || ir_att_bb) {
    ...
    ei.weight[US] += inner_ring_attack[PT] * std::popcount(ir_att_bb) +
                     outer_ring_attack[PT] * std::popcount(or_att_bb);

    if constexpr (PT != PAWN)
      ++ei.n_att[US];
    if constexpr (TRACE)
      T[SAFETY_INNER_RING + PT][US] += std::popcount(ir_att_bb);
    if constexpr (TRACE)
      T[SAFETY_OUTER_RING + PT][US] += std::popcount(or_att_bb);
  }
}

template <Colour US, PieceType PT> void psqt_trace(const Square sq) {
  if constexpr (PT == PAWN)
    ++T[PAWN_PSQT + source32[relative_square(US, sq)]][US];
  if constexpr (PT == KNIGHT)
    ++T[KNIGHT_PSQT + source16[sq]][US];
  if constexpr (PT == BISHOP)
    ++T[BISHOP_PSQT + source16[sq]][US];
  if constexpr (PT == ROOK)
    ++T[ROOK_PSQT + source16[sq]][US];
  if constexpr (PT == QUEEN)
    ++T[QUEEN_PSQT + source32[relative_square(US, sq)]][US];
  if constexpr (PT == KING)
    ++T[KING_PSQT + source16[sq]][US];
}
...
template <Colour US, PieceType PT> Bitboard worthy_trades(const Position &pos) {
  ...
  return (PT == QUEEN) ? pos.pc_bb[make_piece(KING, ~US)] |
                             pos.pc_bb[make_piece(QUEEN, ~US)]
         : (PT == ROOK) ? pos.pc_bb[make_piece(KING, ~US)] |
                              pos.pc_bb[make_piece(QUEEN, ~US)] |
                              pos.pc_bb[make_piece(ROOK, ~US)]
...
void evaluate_majors(const Position &pos, EvalInfo &ei, Score &score) {
  ...
  const Bitboard transparent_occ =
      PT == BISHOP
          ? pos.occ_bb[BOTH] ^ pos.pc_bb[W_QUEEN] ^ pos.pc_bb[B_QUEEN] ^
                pos.pc_bb[make_piece(ROOK, ~US)] ^ ei.ksq[~US]
      : PT == ROOK
          ? pos.occ_bb[BOTH] ^ pos.pc_bb[W_QUEEN] ^ pos.pc_bb[B_QUEEN] ^
                pos.pc_bb[make_piece(ROOK, US)] ^ ei.ksq[~US]
          : pos.occ_bb[BOTH];
  ...
  while (bb) {
    ...
    score +=
        quiet_mobility[PT] * std::popcount(safe_attacks & ~pos.occ_bb[BOTH]);
    score +=
        capture_mobility[PT] * std::popcount(safe_attacks & pos.occ_bb[~US]);
    ...
    if constexpr (TRACE)
      psqt_trace<US, PT>(sq);
    if constexpr (TRACE)
      T[QUIET_MOBILITY + PT][US] +=
          std::popcount(safe_attacks & ~pos.occ_bb[BOTH]);
    if constexpr (TRACE)
      T[CAPTURE_MOBILITY + PT][US] +=
          std::popcount(safe_attacks & pos.occ_bb[~US]);
    if constexpr (PT == KNIGHT) {
    ...
    if constexpr (PT == BISHOP) {
      ...
      if (is_fianchetto<US>(pos, sq)) {
        ...
        if (bitboards::multiple_bits(bitboards::pawn_attacks[US][sq] &
                                     pos.pc_bb[make_piece(PAWN, US)])) {
    ...
    if constexpr (PT == ROOK) {
      if (!(bitboards::file_masks[sq] &
            (pos.pc_bb[W_PAWN] | pos.pc_bb[B_PAWN]))) {
        ...
        if (!(bitboards::file_masks[sq] & pos.pc_bb[make_piece(PAWN, US)])) {
          ...
          if constexpr (TRACE)
            ++T[ROOK_SEMI][US];
        } else if (bitboards::file_masks[sq] &
                   pos.pc_bb[make_piece(PAWN, ~US)]) {
...
Score evaluate_all(const Position &pos, EvalInfo &ei) {
  ...
  if (pos.pc_bb[make_piece(QUEEN, US)] && pos.get_game_phase() > 8) {
    ...
    // we don't count kings or pawns in n_att so the max should be 7, barring
    // promotion trolling
    assert(ei.n_att[US] < 10);

    if (const int mob =
            std::popcount(bitboards::get_attacks<QUEEN>(
                              pos.occ_bb[~US] ^ pos.pc_bb[make_piece(PAWN, US)],
                              ei.ksq[~US]) &
                          ~ei.pawn_attacks[~US]);
        mob > 4) {
      ...
      if constexpr (TRACE)
        T[SAFETY_VIRTUAL_MOBILITY][US] = std::min(13, mob);
    }

    if (ei.weight[US] > 0) {
      score.mg +=
          ei.weight[US] * ei.weight[US] / (720 - attack_factor * ei.n_att[US]);
      if constexpr (TRACE)
        T[SAFETY_N_ATT][US] = ei.n_att[US];
...
Score evaluate_pawns(const Position &pos, EvalInfo &ei) {
  ...
  while (bb) {
    ...
    if (is_passed_pawn<US>(pos.pc_bb[make_piece(PAWN, ~US)], sq)) {
      ...
      if constexpr (TRACE)
        ++T[CANDIDATE_PASSER + relative_rank(US, rank_of(sq))][US];
    }
  ...
  king_danger<US, PAWN, TRACE>(
      bitboards::shift<pawn_push(US)>(pos.pc_bb[make_piece(PAWN, US)]), ei);
  ...
  for (File f = cf - 1; f <= cf + 1; ++f) {
    ...
    if (const Bitboard fp =
            pos.pc_bb[make_piece(PAWN, US)] & bitboards::file_masks[f];
        fp) {
      //	ei.weight[~US] -= *shield_table[US][US == WHITE ?
      // bitboards::lsb(fp) : msb(fp)]; 	if constexpr (TRACE)
      //--T[SAFETY_PAWN_SHIELD + source32[relative_square(US, US == WHITE ?
      // bitboards::lsb(fp) : msb(fp))]][~US];
    } else {
      ei.weight[~US] += shield_table[0][f];
      if constexpr (TRACE)
        ++T[SAFETY_PAWN_SHIELD + source32[f]][~US];
    }
...
template <bool TRACE> int evaluate(const Position &pos) {
  ...
  if (TRACE || ei.key != pos.bs->pawn_key) {
    ...
    ei.score = evaluate_pawns<WHITE, TRACE>(pos, ei) -
               evaluate_pawns<BLACK, TRACE>(pos, ei);
    transposition::new_pawn_entry(ei);
  }

  score += ei.score + evaluate_all<WHITE, TRACE>(pos, ei) -
           evaluate_all<BLACK, TRACE>(pos, ei);

  const int game_phase = pos.get_game_phase();
  int eval =
      (score.mg * game_phase + score.eg * (MAX_GAME_PHASE - game_phase)) /
      MAX_GAME_PHASE;
  if (pos.side == BLACK)
    eval = -eval;
  ...
  popcount(pos.pc_bb[B_BISHOP]) == 1
          && bool(pos.pc_bb[W_BISHOP] & light_mask) == bool(pos.pc_bb[B_BISHOP]
  & dark_mask)) scaling = opposite_bishops_scaling; else scaling = MAX_SCALING;
