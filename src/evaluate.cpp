...
namespace clovis::eval {
    ...
    void evaluate_majors(const Position &pos, EvalInfo &ei, Score &score) {
        ...
        while (bb) {
            ...
            if (pinner != SQ_NONE)
                attacks &=
                    bitboards::between_squares(ei.ksq[US], pinner) | pinner;

            const Bitboard trades = worthy_trades<US, PT>(pos);
            const Bitboard safe_attacks =
                attacks & (~ei.pawn_attacks[~US] | trades);

            score += quiet_mobility[PT] *
                     std::popcount(safe_attacks & ~pos.occ_bb[BOTH]);
            score += capture_mobility[PT] *
            ...
            if constexpr (PT == BISHOP) {
                ...
                if (is_fianchetto<US>(pos, sq)) {
                    ...
                    if (bitboards::multiple_bits(
                            bitboards::pawn_attacks[US][sq] &
                            pos.pc_bb[make_piece(PAWN, US)])) {
            ...
            if constexpr (PT == ROOK) {
                if (!(bitboards::file_masks[sq] &
                    ...
                    if (attacks & rook_on_passer_masks[US][sq] &
                        ei.passers[US]) {
                        score += rook_on_our_passer_file;
                        if constexpr (TRACE) ++T[ROOK_OUR_PASSER][US];
                    }
                    if (safe_attacks & rook_on_passer_masks[~US][sq] &
                        ei.passers[~US]) {
    ...
    Score evaluate_all(const Position &pos, EvalInfo &ei) {
        ...
        if (pos.pc_bb[make_piece(QUEEN, US)] && pos.get_game_phase() > 8) {
            ...
            if (ei.weight[US] > 0) {
                score.mg += ei.weight[US] * ei.weight[US] /
                            (720 - attack_factor * ei.n_att[US]);
    ...
    Score evaluate_pawns(const Position &pos, EvalInfo &ei) {
        ...
        while (bb) {
            ...
            if (is_passed_pawn<US>(pos.pc_bb[make_piece(PAWN, ~US)], sq)) {
                ...
                if (rank_of(sq) != relative_rank(US, RANK_7)) {
                    ...
                    if constexpr (TRACE)
                        ++T[PASSED_PAWN + source32[relative_square(US, sq)]]
                           [US];
                }
        ...
        king_danger<US, PAWN, TRACE>(
            bitboards::shift<pawn_push(US)>(pos.pc_bb[make_piece(PAWN, US)]),
            ei);
