...
namespace clovis::eval {
	...
	Score evaluate_pawns(const Position& pos, EvalInfo& ei) {
		...
		for (File f = cf - 1; f <= cf + 1; ++f) {
			...
			//	ei.weight[~US] -= *shield_table[US][US == WHITE ? bitboards::lsb(fp) : msb(fp)];
			//	if constexpr (TRACE) --T[SAFETY_PAWN_SHIELD + source32[relative_square(US, US == WHITE ? bitboards::lsb(fp) : msb(fp))]][~US];
			} else {
