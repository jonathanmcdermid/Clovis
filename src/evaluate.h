	...
	namespace Eval {
		...
		struct EvalInfo : public PTEntry {
			EvalInfo(PTEntry pte) : PTEntry(pte) {
				attacked[WHITE] = pte.pawn_attacks[WHITE] | Bitboards::king_attacks[pte.ksq[WHITE]];
				attacked[BLACK] = pte.pawn_attacks[BLACK] | Bitboards::king_attacks[pte.ksq[BLACK]];
				n_att[WHITE] = 0;
				n_att[BLACK] = 0;
			}
			void clear() { 
...
#define S(mg, eg) Score(mg, eg)
			...
			S(111, 277), S(149, 256), S(158, 229), S(162, 218),
			S(77, 85), S(94, 80), S(111, 77), S(106, 62),
			S(61, 76), S(79, 71), S(80, 71), S(93, 64),
			S(52, 69), S(62, 69), S(76, 67), S(91, 64),
			S(58, 64), S(71, 64), S(75, 69), S(78, 72),
			S(54, 65), S(77, 64), S(70, 76), S(71, 76),
			S(0, 0), S(0, 0), S(0, 0), S(0, 0),
		};

		constexpr Score knight_table[] = {
			S(173, 184), S(239, 200), S(222, 226), S(298, 213),
			S(272, 207), S(295, 224), S(373, 206), S(326, 231),
			S(308, 206), S(354, 215), S(341, 237), S(354, 233),
			S(325, 218), S(326, 234), S(335, 245), S(337, 250),
			S(315, 218), S(329, 232), S(335, 243), S(335, 247),
			S(310, 216), S(332, 224), S(334, 229), S(339, 241),
			S(311, 201), S(308, 218), S(324, 225), S(333, 227),
			S(286, 198), S(315, 192), S(301, 217), S(316, 217),
		};

		constexpr Score bishop_table[] = {
			S(311, 223), S(328, 227), S(329, 224), S(326, 229),
			S(313, 225), S(344, 215), S(341, 229), S(341, 230),
			S(323, 231), S(341, 232), S(350, 233), S(340, 239),
			S(322, 233), S(329, 232), S(329, 239), S(344, 237),
		};

		constexpr Score rook_table[] = {
			S(439, 419), S(432, 426), S(441, 424), S(442, 422),
			S(413, 430), S(430, 427), S(438, 425), S(447, 420),
			S(418, 429), S(437, 428), S(439, 424), S(437, 424),
			S(416, 433), S(434, 428), S(441, 430), S(441, 425),
		};

		constexpr Score queen_table[] = {
			S(886, 779), S(871, 802), S(879, 810), S(888, 801),
			S(884, 780), S(847, 811), S(868, 813), S(839, 843),
			S(897, 779), S(885, 797), S(890, 797), S(868, 829),
			S(876, 808), S(871, 818), S(873, 809), S(865, 822),
			S(894, 777), S(881, 812), S(882, 804), S(881, 812),
			S(889, 791), S(904, 775), S(892, 797), S(894, 789),
			S(887, 774), S(899, 760), S(911, 760), S(905, 774),
			S(904, 753), S(894, 758), S(894, 760), S(907, 750),
		};

		constexpr Score king_table[] = {
			S(74, 18), S(106, 38), S(67, 61), S(73, 58),
			S(81, 42), S(108, 55), S(75, 76), S(51, 86),
			S(56, 55), S(92, 69), S(77, 83), S(66, 91),
			S(22, 59), S(68, 77), S(55, 93), S(39, 101),
		};

		constexpr Score passed_pawn[] = {
			S(0, 0), S(0, 0), S(0, 0), S(0, 0),
			S(0, 0), S(0, 0), S(0, 0), S(0, 0),
			S(11, 120), S(2, 116), S(17, 89), S(9, 84),
			S(14, 61), S(7, 59), S(10, 40), S(5, 33),
			S(10, 34), S(0, 33), S(0, 18), S(0, 16),
			S(6, 12), S(0, 14), S(0, 4), S(0, 2),
			S(0, 12), S(1, 10), S(5, 0), S(0, 2),
			S(0, 0), S(0, 0), S(0, 0), S(0, 0),
		};

		constexpr Score candidate_passer[] = {
			S(0, 0), S(0, 1), S(0, 4), S(5, 17), S(11, 36), S(21, 48), S(0, 0), S(0, 0),
		};

		constexpr Score mobility[] = {
			S(0, 0), S(0, 0), S(5, 0), S(5, 2), S(4, 2), S(2, 3), S(0, 0),
		};

		constexpr Score double_pawn_penalty = S(0, 6);
		constexpr Score isolated_pawn_penalty = S(14, 7);
		constexpr Score bishop_pair_bonus = S(22, 41);
		...
		constexpr Score king_adjacent_full_open_penalty = S(4, 9);
		...
		constexpr Score rook_on_their_passer_file = S(3, 41);
		constexpr Score tall_pawn_penalty = S(12, 23);
		constexpr Score fianchetto_bonus = S(18, 12);
		constexpr Score rook_on_seventh = S(6, 22);
