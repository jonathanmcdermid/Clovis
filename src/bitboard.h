...
constexpr auto empty_bishop_attacks = [] {
    ...
    for (Square s1 = SQ_ZERO; s1 < SQ_N; ++s1)
        for (const auto d1 : {NORTH, SOUTH})
            for (const auto d2 : {EAST, WEST})
                for (Square s2 = s1 + d1 + d2; is_valid(s2) && rank_of(s2) == rank_of(s2 - d2);
                     s2 += d1 + d2)
                    arr[s1] |= s2;
...
constexpr auto bishop_masks = [] {
    ...
    for (Square s1 = SQ_ZERO; s1 < SQ_N; ++s1)
        for (const auto d1 : {NORTH, SOUTH})
            for (const auto d2 : {EAST, WEST})
                for (Square s2 = s1; is_valid(s2 + d1) && rank_of(s2) == rank_of(s2 + d2);
                     s2 += d1 + d2)
                    if (s2 != s1) arr[s1] |= s2;
...
constexpr Bitboard get_attacks(const Bitboard occ, const Square sq) {
    ...
    return PT == KNIGHT ? knight_attacks[sq]
           : PT == BISHOP
               ? bishop_attacks[sq]
                               [(occ & bishop_masks[sq]) * bishop_magic[sq] >> bishop_bit_count[sq]]
           : PT == ROOK
               ? rook_attacks[sq][(occ & rook_masks[sq]) * rook_magic[sq] >> rook_bit_count[sq]]
