...
constexpr bool multiple_bits(const Bitboard bb) { return bb & (bb - 1); }
    ...
    for (const auto d1 : {NORTH, SOUTH})
      for (const auto d2 : {EAST, WEST})
        for (Square s2 = s1 + d1 + d2;
             is_valid(s2) && rank_of(s2) == rank_of(s2 - d2); s2 += d1 + d2)
          arr[s1] |= s2;
...
constexpr auto between_bb = [] {
  ...
  for (Square sq1 = SQ_ZERO; sq1 < SQ_N; ++sq1) {
    for (Square sq2 = SQ_ZERO; sq2 < SQ_N; ++sq2) {
      ...
      while (bb)
        if (const Square sq3 = pop_lsb(bb); (sq1 < sq3) == (sq3 < sq2))
          arr[sq1][sq2] |= sq3;
    }
  }
...
constexpr Bitboard get_attacks(const Bitboard occ, const Square sq) {
  ...
  return PT == KNIGHT ? knight_attacks[sq]
         : PT == BISHOP
             ? bishop_attacks[sq][(occ & bishop_masks[sq]) * bishop_magic[sq] >>
                                  bishop_bit_count[sq]]
         : PT == ROOK
