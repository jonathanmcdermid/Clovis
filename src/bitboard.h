...
namespace clovis::bitboards {
        ...
        for (Square sq = SQ_ZERO; sq < SQ_N; ++sq) {
            for (const auto d1 : {NORTH, SOUTH}) {
                for (const auto d2 : {EAST, WEST}) {
                    if (is_valid(sq + d1 + d1) &&
                        rank_of(sq) == rank_of(sq + d2))
                        arr[sq] |= sq + d1 + d1 + d2;
                    if (is_valid(sq + d1) &&
                        rank_of(sq) == rank_of(sq + d2 + d2))
                        arr[sq] |= sq + d1 + d2 + d2;
                }
            }
        }
    ...
    constexpr auto empty_bishop_attacks = [] {
        ...
        for (Square s1 = SQ_ZERO; s1 < SQ_N; ++s1)
            for (const auto d1 : {NORTH, SOUTH})
                for (const auto d2 : {EAST, WEST})
                    for (Square s2 = s1 + d1 + d2;
                         is_valid(s2) && rank_of(s2) == rank_of(s2 - d2);
                         s2 += d1 + d2)
                        arr[s1] |= s2;
        ...
        for (Square s1 = SQ_ZERO; s1 < SQ_N; ++s1) {
            for (const auto dir : {NORTH, SOUTH})
                for (Square s2 = s1 + dir; is_valid(s2); s2 += dir)
                    arr[s1] |= s2;
            for (const auto dir : {EAST, WEST})
                for (Square s2 = s1 + dir; rank_of(s2) == rank_of(s2 - dir);
                     s2 += dir)
                    arr[s1] |= s2;
        }
    ...
    constexpr auto bishop_masks = [] {
        ...
        for (Square s1 = SQ_ZERO; s1 < SQ_N; ++s1)
            for (const auto d1 : {NORTH, SOUTH})
                for (const auto d2 : {EAST, WEST})
                    for (Square s2 = s1;
                         is_valid(s2 + d1) && rank_of(s2) == rank_of(s2 + d2);
                         s2 += d1 + d2)
                        if (s2 != s1) arr[s1] |= s2;
        ...
        for (Square s1 = SQ_ZERO; s1 < SQ_N; ++s1) {
            for (const auto dir : {NORTH, SOUTH})
                for (Square s2 = s1; is_valid(s2 + dir); s2 += dir)
                    if (s2 != s1) arr[s1] |= s2;
            for (const auto dir : {EAST, WEST})
                for (Square s2 = s1; rank_of(s2) == rank_of(s2 + dir);
                     s2 += dir)
                    if (s2 != s1) arr[s1] |= s2;
        }
    ...
    extern std::array<std::array<Bitboard, bishop_attack_indices>, SQ_N>
        bishop_attacks;
    extern std::array<std::array<Bitboard, rook_attack_indices>, SQ_N>
        rook_attacks;

    void print_bitboard(const Bitboard &bb);
    void init_bitboards();

    constexpr Bitboard between_squares(const Square sq1, const Square sq2) {
    ...
    constexpr Bitboard get_attacks(const Bitboard occ, const Square sq) {
        ...
        return PT == KNIGHT   ? knight_attacks[sq]
               : PT == BISHOP ? bishop_attacks[sq][(occ & bishop_masks[sq]) *
                                                       bishop_magic[sq] >>
                                                   bishop_bit_count[sq]]
               : PT == ROOK
                   ? rook_attacks[sq][(occ & rook_masks[sq]) * rook_magic[sq] >>
                                      rook_bit_count[sq]]
               : PT == QUEEN
                   ? get_attacks<BISHOP>(occ, sq) | get_attacks<ROOK>(occ, sq)
                   : king_attacks[sq];
    }
    ...
    constexpr Bitboard get_attacks(const PieceType pt, const Bitboard occ,
                                   const Square sq) {
        assert(pt != PAWN);
