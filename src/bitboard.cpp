...
namespace clovis::bitboards {
    ...
    std::array<std::array<Bitboard, bishop_attack_indices>, SQ_N>
                                                                bishop_attacks;
    ...
    Bitboard set_occupancy(Bitboard attack_mask, const int index,
                           const int bits) {
        Bitboard occ = 0ULL;
    ...
    void init_bitboards() {
        for (Square sq = SQ_ZERO; sq < SQ_N; ++sq) {
            for (int index = 0; index < (1 << std::popcount(bishop_masks[sq]));
                 ++index) {
                const Bitboard occ = set_occupancy(
                    bishop_masks[sq], index, std::popcount(bishop_masks[sq]));
                bishop_attacks[sq][(occ * bishop_magic[sq]) >>
                                   bishop_bit_count[sq]] = bishop_otf(sq, occ);
            }

            for (int index = 0; index < (1 << std::popcount(rook_masks[sq]));
                 ++index) {
                const Bitboard occ = set_occupancy(
                    rook_masks[sq], index, std::popcount(rook_masks[sq]));
                rook_attacks[sq][(occ * rook_magic[sq]) >> rook_bit_count[sq]] =
                    rook_otf(sq, occ);
            }
        }
    }
