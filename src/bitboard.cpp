...
void init_bitboards() {
  ...
  for (Square sq = SQ_ZERO; sq < SQ_N; ++sq) {
    for (int index = 0; index < (1 << std::popcount(bishop_masks[sq]));
         ++index) {
      const Bitboard occ = set_occupancy(bishop_masks[sq], index,
                                         std::popcount(bishop_masks[sq]));
      bishop_attacks[sq][(occ * bishop_magic[sq]) >> bishop_bit_count[sq]] =
          bishop_otf(sq, occ);
    }

    for (int index = 0; index < (1 << std::popcount(rook_masks[sq])); ++index) {
      const Bitboard occ =
          set_occupancy(rook_masks[sq], index, std::popcount(rook_masks[sq]));
