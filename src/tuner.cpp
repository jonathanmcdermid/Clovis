...
void init_params() {
  ...
  for (auto &it : pawn_source)
    add_param<Score>(it,
                     static_cast<TraceIndex>(&it + PAWN_PSQT - pawn_source));
  ...
  for (auto &it : rook_source)
    add_param<Score>(it,
                     static_cast<TraceIndex>(&it + ROOK_PSQT - rook_source));
  for (auto &it : queen_source)
    add_param<Score>(it,
                     static_cast<TraceIndex>(&it + QUEEN_PSQT - queen_source));
  for (auto &it : king_source)
    add_param<Score>(it,
                     static_cast<TraceIndex>(&it + KING_PSQT - king_source));
  for (auto &it : passed_pawn)
    add_param<Score>(it,
                     static_cast<TraceIndex>(&it + PASSED_PAWN - passed_pawn));
  for (auto &it : candidate_passer)
    add_param<Score>(
        it, static_cast<TraceIndex>(&it + CANDIDATE_PASSER - candidate_passer));
  for (auto &it : quiet_mobility)
    add_param<Score>(
        it, static_cast<TraceIndex>(&it + QUIET_MOBILITY - quiet_mobility));
  for (auto &it : capture_mobility)
    add_param<Score>(
        it, static_cast<TraceIndex>(&it + CAPTURE_MOBILITY - capture_mobility));
  ...
  for (auto &it : pawn_shield)
    add_param<short>(
        it, static_cast<TraceIndex>(&it + SAFETY_PAWN_SHIELD - pawn_shield));
  for (auto &it : inner_ring_attack)
    add_param<short>(it, static_cast<TraceIndex>(&it + SAFETY_INNER_RING -
                                                 inner_ring_attack));
  for (auto &it : outer_ring_attack)
    add_param<short>(it, static_cast<TraceIndex>(&it + SAFETY_OUTER_RING -
                                                 outer_ring_attack));
...
template <bool STATIC> double mse(const double k) {
  ...
  auto compute_chunk = [&](std::size_t start, std::size_t end) {
    for (std::size_t i = start; i < end; ++i) {
      ...
      total +=
          std::pow(it.result - sigmoid(k, (STATIC ? it.static_eval
  ...
  for (int i = 0; i < n_threads; ++i)
    threads[i] =
        std::thread(compute_chunk, i * chunk_size, i * chunk_size + chunk_size);
...
void print_table(const std::string &name, const int index, const int size,
  ...
  std::cout << "\t\tconstexpr" << ((index < TI_SAFETY) ? " Score " : " short ")
            << name << "[] = {" << std::endl
            << "\t\t";
...
void print_params() {
  ...
  print_table("queen_source", QUEEN_PSQT, sizeof(queen_source) / sizeof(Score),
              4);
  print_table("king_source", KING_PSQT, sizeof(king_source) / sizeof(Score), 4);
  print_table("passed_pawn", PASSED_PAWN, sizeof(passed_pawn) / sizeof(Score),
              4);
...
double find_k() {
  ...
  for (int epoch = 0; epoch < 10; ++epoch) {
    ...
    std::cout << "Epoch [" << epoch << "] Error = [" << best << "], K = ["
              << start << "]" << std::endl;
