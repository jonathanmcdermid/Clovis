...
namespace clovis::tuner {
    ...
    void init_params() {
        ...
        for (auto &it : pawn_source)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + PAWN_PSQT - pawn_source));
        for (auto &it : knight_source)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + KNIGHT_PSQT - knight_source));
        for (auto &it : bishop_source)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + BISHOP_PSQT - bishop_source));
        for (auto &it : rook_source)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + ROOK_PSQT - rook_source));
        for (auto &it : queen_source)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + QUEEN_PSQT - queen_source));
        for (auto &it : king_source)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + KING_PSQT - king_source));
        for (auto &it : passed_pawn)
            add_param<Score>(
                it, static_cast<TraceIndex>(&it + PASSED_PAWN - passed_pawn));
        for (auto &it : candidate_passer)
            add_param<Score>(it,
                             static_cast<TraceIndex>(&it + CANDIDATE_PASSER -
                                                     candidate_passer));
        for (auto &it : quiet_mobility)
            add_param<Score>(it, static_cast<TraceIndex>(&it + QUIET_MOBILITY -
                                                         quiet_mobility));
        for (auto &it : capture_mobility)
            add_param<Score>(it,
                             static_cast<TraceIndex>(&it + CAPTURE_MOBILITY -
                                                     capture_mobility));
        ...
        for (auto &it : pawn_shield)
            add_param<short>(it, static_cast<TraceIndex>(
                                     &it + SAFETY_PAWN_SHIELD - pawn_shield));
        for (auto &it : inner_ring_attack)
            add_param<short>(it,
                             static_cast<TraceIndex>(&it + SAFETY_INNER_RING -
                                                     inner_ring_attack));
        for (auto &it : outer_ring_attack)
            add_param<short>(it,
                             static_cast<TraceIndex>(&it + SAFETY_OUTER_RING -
                                                     outer_ring_attack));
    ...
    double linear_eval(const TEntry &entry, TGradient *tg) {
        ...
        const double eval = ((normal[MG] + safety) * entry.phase +
                             normal[EG] * (MAX_GAME_PHASE - entry.phase)) /
                            MAX_GAME_PHASE;
    ...
    double mse(const double k) {
        ...
        auto compute_chunk = [&](std::size_t start, std::size_t end) {
            for (std::size_t i = start; i < end; ++i) {
                ...
                total += std::pow(
                    it.result - sigmoid(k, (STATIC ? it.static_eval
                                                   : linear_eval(it, nullptr))),
                    2);
            }
        };

        for (int i = 0; i < n_threads; ++i)
            threads[i] = std::thread(compute_chunk, i * chunk_size,
    ...
    void update_single_gradient(const TEntry &entry, TVector &gradient,
                                const double k) {
        TGradient tg;
        ...
        const double base[PHASE_N] = {a * entry.phase,
                                      a * (MAX_GAME_PHASE - entry.phase)};

        for (auto &it : entry.tuples) {
            if (it.index < TI_SAFETY) {
                gradient[it.index][MG] +=
                    base[MG] * (it.coefficient[WHITE] - it.coefficient[BLACK]);
                gradient[it.index][EG] +=
                    base[EG] * (it.coefficient[WHITE] - it.coefficient[BLACK]);
            } else {
                gradient[it.index][MG] +=
                    2 * base[MG] * tg.safety[WHITE] * it.coefficient[WHITE] /
                    (720.0 - params[SAFETY_N_ATT][MG] * entry.n_att[WHITE]);
                gradient[it.index][MG] -=
                    2 * base[MG] * tg.safety[BLACK] * it.coefficient[BLACK] /
                    (720.0 - params[SAFETY_N_ATT][MG] * entry.n_att[BLACK]);
            }
        }

        gradient[SAFETY_N_ATT][MG] +=
            base[MG] * pow(tg.safety[WHITE], 2.0) * entry.n_att[WHITE] /
            pow(720.0 - params[SAFETY_N_ATT][MG] * entry.n_att[WHITE], 2.0);
        gradient[SAFETY_N_ATT][MG] -=
            base[MG] * pow(tg.safety[BLACK], 2.0) * entry.n_att[BLACK] /
            pow(720.0 - params[SAFETY_N_ATT][MG] * entry.n_att[BLACK], 2.0);
    }
    ...
    void print_table(const std::string &name, const int index, const int size,
                     const int cols) {
        std::cout << "\t\tconstexpr"
                  << ((index < TI_SAFETY) ? " Score " : " short ") << name
                  << "[] = {" << std::endl
                  << "\t\t";
    ...
    void print_params() {
        ...
        print_table("pawn_source", PAWN_PSQT,
                    sizeof(pawn_source) / sizeof(Score), 4);
        print_table("knight_source", KNIGHT_PSQT,
                    sizeof(knight_source) / sizeof(Score), 4);
        print_table("bishop_source", BISHOP_PSQT,
                    sizeof(bishop_source) / sizeof(Score), 4);
        print_table("rook_source", ROOK_PSQT,
                    sizeof(rook_source) / sizeof(Score), 4);
        print_table("queen_source", QUEEN_PSQT,
                    sizeof(queen_source) / sizeof(Score), 4);
        print_table("king_source", KING_PSQT,
                    sizeof(king_source) / sizeof(Score), 4);
        print_table("passed_pawn", PASSED_PAWN,
                    sizeof(passed_pawn) / sizeof(Score), 4);
        print_table("candidate_passer", CANDIDATE_PASSER,
                    sizeof(candidate_passer) / sizeof(Score), 8);
        print_table("quiet_mobility", QUIET_MOBILITY, 7, 7);
        print_table("capture_mobility", CAPTURE_MOBILITY, 7, 7);

        std::cout << "\t\tconstexpr Score double_pawn_penalty = "
                  << Score(params[DOUBLE_PAWN]) << ";" << std::endl
                  << "\t\tconstexpr Score isolated_pawn_penalty = "
                  << Score(params[ISOLATED_PAWN]) << ";" << std::endl
                  << "\t\tconstexpr Score bishop_pair_bonus = "
                  << Score(params[BISHOP_PAIR]) << ";" << std::endl
                  << "\t\tconstexpr Score rook_open_file_bonus = "
                  << Score(params[ROOK_FULL]) << ";" << std::endl
                  << "\t\tconstexpr Score rook_semi_open_file_bonus = "
                  << Score(params[ROOK_SEMI]) << ";" << std::endl
                  << "\t\tconstexpr Score rook_closed_file_penalty = "
                  << Score(params[ROOK_CLOSED]) << ";" << std::endl
                  << "\t\tconstexpr Score tempo_bonus = "
                  << Score(params[TEMPO]) << ";" << std::endl
                  << "\t\tconstexpr Score king_open_penalty = "
                  << Score(params[KING_OPEN]) << ";" << std::endl
                  << "\t\tconstexpr Score king_adjacent_open_penalty = "
                  << Score(params[KING_ADJ_OPEN]) << ";" << std::endl
                  << "\t\tconstexpr Score knight_outpost_bonus = "
                  << Score(params[KNIGHT_OUTPOST]) << ";" << std::endl
                  << "\t\tconstexpr Score bishop_outpost_bonus = "
                  << Score(params[BISHOP_OUTPOST]) << ";" << std::endl
                  << "\t\tconstexpr Score weak_queen_penalty = "
                  << Score(params[WEAK_QUEEN]) << ";" << std::endl
                  << "\t\tconstexpr Score rook_on_our_passer_file = "
                  << Score(params[ROOK_OUR_PASSER]) << ";" << std::endl
                  << "\t\tconstexpr Score rook_on_their_passer_file = "
                  << Score(params[ROOK_THEIR_PASSER]) << ";" << std::endl
                  << "\t\tconstexpr Score tall_pawn_penalty = "
                  << Score(params[TALL_PAWN]) << ";" << std::endl
                  << "\t\tconstexpr Score fianchetto_bonus = "
                  << Score(params[FIANCHETTO]) << ";" << std::endl
                  << "\t\tconstexpr Score rook_on_seventh = "
                  << Score(params[ROOK_ON_SEVENTH]) << ";" << std::endl
                  << std::endl;

        print_table("pawn_shield", SAFETY_PAWN_SHIELD,
                    sizeof(pawn_shield) / sizeof(short), 4);
        print_table("inner_ring_attack", SAFETY_INNER_RING, 7, 7);
        print_table("outer_ring_attack", SAFETY_OUTER_RING, 7, 7);

        std::cout << "\t\tconstexpr short attack_factor = "
                  << round(params[SAFETY_N_ATT][MG]) << ";" << std::endl
                  << "\t\tconstexpr short virtual_mobility = "
                  << round(params[SAFETY_VIRTUAL_MOBILITY][MG]) << ";"
                  << std::endl;
    }
    ...
    double find_k() {
        ...
        for (int epoch = 0; epoch < 10; ++epoch) {
            ...
            std::cout << "Epoch [" << epoch << "] Error = [" << best
                      << "], K = [" << start << "]" << std::endl;
    ...
    void tune_eval() {
        ...
        while (getline(ifs, line)) {
            ...
            if (const std::string res = line.substr(idx + 1, idx_end - idx - 1);
                res == "1-0")
                entry.result = 1.0;
            else if (res == "0-1")
                entry.result = 0.0;
            ...
            for (int j = 0; j < TI_N; ++j)
                if ((j < TI_SAFETY &&
                     eval::T[j][WHITE] - eval::T[j][BLACK] != 0) ||
                    (j >= TI_SAFETY &&
                     (eval::T[j][WHITE] != 0 || eval::T[j][BLACK] != 0)))
                    entry.tuples.emplace_back(j, eval::T[j][WHITE],
                                              eval::T[j][BLACK]);
        ...
        for (int epoch = 1; epoch < MAX_EPOCHS; ++epoch) {
            ...
            for (size_t i = 0; i < TI_MISC; ++i) {
                adaptive_gradient[i][MG] +=
                    pow((k / 200.0) * gradient[i][MG] / 16384, 2.0);
                adaptive_gradient[i][EG] +=
                    pow((k / 200.0) * gradient[i][EG] / 16384, 2.0);
            ...
            std::cout << "Epoch [" << epoch << "] Error = [" << mse<false>(k)
                      << "], Rate = [" << rate << "]" << std::endl;
        }
    }
